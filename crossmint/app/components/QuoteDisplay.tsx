...
import type { RelayQuoteResponse } from "../actions/relay";
...
export function QuoteDisplay({ quote }: QuoteDisplayProps) {
  ...
  // Format USD amounts to two decimal places
  const formatUsd = (val: string) => `$${parseFloat(val).toFixed(2)}`;

  // Format the time estimate into a human-readable string
  const timeLabel =
    timeEstimate < 60
      ? `~${timeEstimate}s`
          ...
          <Row
            ...
            sub={
              fees.relayer.amountUsd
                ? formatUsd(fees.relayer.amountUsd)
                : undefined
            }
          />
        )}
...
function Row({
  ...
  return (
    <div className="flex items-baseline justify-between">
      ...
      <span className={highlight ? "font-semibold text-green-400" : ""}>
        {value}
        {sub && (
          <span className="ml-1.5 text-xs text-gray-500">{sub}</span>
        )}
      </span>
    </div>
  );
